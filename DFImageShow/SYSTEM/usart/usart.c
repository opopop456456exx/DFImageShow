...
u8 USART_RX_HEAD_BUF[8];            //帧头解码缓冲，协议8字节
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
FIL fil;
u32 USART_RX_STA=0,USART_RX_LEN=0;       //接收状态标记	,解码接收字节数  
u8 RxTimeout=0;             //接收超时 ，tim3定时器50ms，150ms没有进入串口接收中断视为超时。目前写的可能有bug还没发现
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
	...
	//	while(USART_GetFlagStatus(USART1,USART_FLAG_TC)!=SET);//等待发送结束
			...
			if(w==8)
			{
				if((USART_RX_HEAD_BUF[0]== 0xDF)&&(USART_RX_HEAD_BUF[1]==0xDF)&&(USART_RX_HEAD_BUF[2]==0xFD)&&(USART_RX_HEAD_BUF[3]==0xFD))    //帧头DFDFFDFD
				{
					USART_RX_LEN=((u32)USART_RX_HEAD_BUF[4]<<24)|((u32)USART_RX_HEAD_BUF[5]<<16)|((u32)USART_RX_HEAD_BUF[6]<<8)|((u32)USART_RX_HEAD_BUF[7]);
					if(USART_RX_LEN!=0)
					{
						USART_RX_STA|=0x20000;                      //帧头校验成功
					}
				}
				w=0;
			}
			...
			else                                               //帧头校验成功,开始接收数据
			{
				...
				USART_RX_BUF[USART_RX_STA&0X1FFFF]=Res ;
					// f_write (&fil, Res, 1, &CRes);       直接读不行，耗时，会中断通信
					USART_RX_STA++;
           ...
           USART_RX_STA|=0x40000;                         //数据接收完了，  进入  末尾oxod,oxoa的校验
				}
			}
				else
				{
					...
					if(w==2)
					{
						if((USART_RX_HEAD_BUF[0]== 0x0d)&&(USART_RX_HEAD_BUF[1]==0x0a))
						{
							...
							f_close(&fil); //f_close 很关键，写完数据立刻close，显示才会正常,因为每次要重新打开创建。

...
{			   
	...
	//	if(tp_dev.c2<=1){
		...
		{		 
			tp_dev.sta=TP_PRES_DOWN|TP_CATH_PRES;//按键按下     11000000
	...
	else
	{
		...
		{
			tp_dev.sta&=~(1<<7);//标记按键松开	                0-------
			if(tp_dev.c2==0){
				tp_dev.x[1]=tp_dev.x[0];
			tp_dev.y[1]=tp_dev.y[0];  	   
			}
			if(tp_dev.c2==1){
				tp_dev.x[2]=tp_dev.x[0];
			 tp_dev.y[2]=tp_dev.y[0];  	   
			}
...
{
	AT24CXX_Write(SAVE_ADDR_BASE,(u8*)&tp_dev.xfac,14);	//强制保存&tp_dev.xfac地址开始的14个字节数据，即保存到tp_dev.touchtype、//为什么是14？
...
{	
	...
	}else
	{
	  GPIO_InitTypeDef  GPIO_InitStructure;
		...
		TP_Read_XY(&tp_dev.x[0],&tp_dev.y[0]);//第一次读取初始化	  这个AD转换读取是SPI方式
		AT24CXX_Init();			//初始化24CXX   EEPROM存储
